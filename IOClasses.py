...
# Evan Anderson, 2021-Dec-12, Built out methods (save_inventory, load_inventory)
#------------------------------------------#
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_trk, 'r') as file:
                for line in file:
                    data = line.strip().split(',') # Converts track string into list
                    cd = PC.DataProcessor.select_cd(lst_Inventory, int(data[0])) # Selects a CD object from the CD object list with an ID that matches the track ID
                    track = DC.Track(int(data[1]), data[2], data[3]) # Creates the track object by assigning values to attributes using current list
                    cd.add_track(track) # Appends track object to selected CD object's track list and sorts track by track ID
            return lst_Inventory
        except FileNotFoundError:
            print('Source files not found! New text documents created.\n')
            objFile1 = open(file_name_CD, 'w')
            objFile2 = open(file_name_trk, 'w')
...
class ScreenIO:
    ...
    def menu_choice():
        ...
        choice = ' '
        while True:
            choice = input('Which operation would you like to perform? [l, a, d, c, s, x]: ').lower().strip()
            OneOfMenuOptions = choice in ['l', 'a', 'd', 'c', 's', 'x']
            if OneOfMenuOptions:
                break
            try:
    ...
    def get_CD_info():
        ...
        return cdId, cdTitle, cdArtist # packs tuple based on user input
